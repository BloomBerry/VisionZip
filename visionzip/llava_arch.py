...
def encode_images_visionzip_clip(self, images):
    image_features, keep_idx = self.get_model().get_vision_tower(load_model = "clip").forward(images)
    image_features = self.get_model().mm_projector(image_features)        
    return image_features, keep_idx 

def encode_images_visionzip_dino(self, images, all_indices):
    image_features = self.get_model().get_vision_tower(load_model = "dino").forward(images, all_indices)
    image_features = self.get_model().mm_projector(image_features)        
    return image_features
...
def prepare_inputs_labels_for_multimodal_withdino_visionzip(
    ...
    if type(images) is list or images.ndim == 5:
        ...
        image_features_clip, all_indices = self.encode_images_visionzip_clip(images)
        image_features_dino = self.encode_images_visionzip_dino(images, all_indices)
    ...
    for batch_idx, cur_input_ids in enumerate(input_ids):
        ...
        if num_images == 0:
            ...
            cur_input_embeds_1 = self.get_model().embed_tokens(cur_input_ids)
            cur_input_embeds = torch.cat([cur_input_embeds_1, cur_image_features_clip[0:0], cur_image_features_dino[0:0]], dim=0)
            new_input_embeds.append(cur_input_embeds)
            new_labels.append(labels[batch_idx])
            cur_image_idx += 1
            continue
